...
// Input :
//      (1) ROOT_TRIE of trie (¿Ã∞Õ¿∫ ∆ƒ∂Û∏ﬁ≈∏∑Œ ≥—∞‹¡ˆ¥¬ ∞Õ¿Ã æ∆¥œ∞Ì «‘ºˆ Ω√¿€∫Œ∫–ø°º≠ *curr = ROOT_TRIE πÆ¿Âø°
//          ¿««ÿ ∑Á∆Æ∫Œ≈Õ √£±‚ Ω√¿€«œµµ∑œ «‘¿ª ¿«πÃ)
//      (2) a string of key (√£¿ª ≈∞¿« πÆ¿⁄ø≠)
// Output :
//    (1) integer return value : -1: √£±‚∞° º∫∞¯«— ∞ÊøÏ;  0 ¿ÃªÛ ¡§ºˆ:  √£±‚∞° Ω«∆–«— ∞ÊøÏ Ω«∆–«— ±€¿⁄¿« index.
...
nodeptr  insert_trie(char key[50]) {
    int res = 0, i = 0;
    char c;
    nodeptr tn, curr, prev, node_found;
    ...
    if (prev == NULL) {
        ...
        tn->right = ROOT_TRIE;
        ROOT_TRIE = tn;
        node_found = hang_down(key, tn, i + 1);
    }
...
void  substitution(nodeptr root_sub_trie, char key[], char  corr_key[], int ki, int ci, int n_s, int n_d, int n_i, int n_t)
{
    ...
    int temp;
    char  new_corr_key[100];
    strcpy(new_corr_key, corr_key); // ªı∑ŒøÓ ±≥¡§µ» πÆ¿⁄ø≠ ∞¯∞£¿ª ∏∏µÈæÓ ¡ˆ±›±Ó¡ˆ¿« ∞·∞˙∏¶ ¿˙¿Â«—¥Ÿ.
    np = root_sub_trie;
    do {
        start_ptr = np->below;
...
void main() {
    ...
    int ki = 0, ci = 0; // ki: ¥Ÿ¿Ω √≥∏Æ«“ key ¿« ±€¿⁄ ¿Œµ¶Ω∫;    ci: ¥Ÿ¿Ωø° ≥÷¿ª corr_key ¿« ¿Œµ¶Ω∫.
